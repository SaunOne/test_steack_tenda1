...
> = {
  ...
  GET: async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
      if (transaksi_id) {
        ...
        return res.status(200).json(transaksi); // Return single transaction object
      }
  ...
  POST: async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
      ...
      if (
        applicableLevelMember &&
        pelanggan.level_member_id !== applicableLevelMember.level_member_id
      ) {
      ...
      for (const item of menu) {
        ...
        if (!menuItem) {
          return res
            .status(404)
            .json({ error: `Menu with id ${item.id_menu} not found` });
        }
      ...
      const newTransaksi = await prisma.transaksi.create({
        data: {
          ...
          total_diskon: Math.round(totalHarga * diskonRate), // Simpan total diskon
        },
      });
...
export default async function handler(
  ...
  const methodHandler =
    methods[req.method as "GET" | "POST" | "PUT" | "DELETE"];
  if (methodHandler) {
